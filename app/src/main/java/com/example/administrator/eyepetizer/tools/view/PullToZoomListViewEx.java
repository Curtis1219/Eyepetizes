...
public class PullToZoomListViewEx extends PullToZoomBase<ListView> implements AbsListView.OnScrollListener {
    ...
    protected void smoothScrollToTop() {
        ...
        mScalingRunnable.startAnimation(500L);
    }

    @Override
    protected void pullHeaderToZoom(int newScrollValue,int newScrollValueX) {
        ...
        localLayoutParams.width = Math.abs(newScrollValueX) + mHeaderWidth;

        float w = mHeaderView.getMeasuredWidth()/2 > mHeaderWidth/2 ? mHeaderView.getMeasuredWidth()/2 : mHeaderView.getWidth()/2;
        float h = mHeaderView.getMeasuredHeight() > mHeaderHeight/2 ? mHeaderView.getMeasuredHeight()/2 : mHeaderView.getHeight()/2;


        View childAt = mHeaderContainer.getChildAt(0);
        mHeaderContainer.getRootView().setRotationX(w);
        mHeaderContainer.getRootView().setRotationY(h);
...
//                        ((MarginLayoutParams) localLayoutParams).setMargins(-(localLayoutParams.width - mHeaderWidth) / 2, 0, 0, 0);
//                        ((MarginLayoutParams) localLayoutParams).setMargins();
//                        ((MarginLayoutParams) localLayoutParams).leftMargin
//                        self.imageView.frame.origin.y
//                        View inflate = View.inflate(mHeaderContainer.getContext(), mHeaderContainer, this);
//                        mHeaderContainer.setScrollBarSize();
//                        Rect rect = new Rect(-(localLayoutParams.width - mHeaderWidth) / 2, 0, 0, 0);
...
//                        self.iv.centerY=scrollView.contentSize.height > scrollView.frame.size.height ? scrollView.contentSize.height/2 :self.sc.centerY;
                        mHeaderContainer.getRootView().setPivotX(w);
                        mHeaderContainer.getRootView().setPivotY(h);
